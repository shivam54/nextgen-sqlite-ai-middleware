...
/* ADS TODO : writing the helper function here */

#define SQLITE_TBL_SIZE_SMALL   1
#define SQLITE_TBL_SIZE_MEDIUM  2
#define SQLITE_TBL_SIZE_LARGE   3

/* Classify a table by estimated row count nRow (LogEst). */
static int sqlite3TableSizeClass(LogEst nRow){
  /* Thresholds are approximate; any reasonable LogEst for 10 and 15 works. [web:1][web:5] */
  static const LogEst log10 = 33;  /* ~10 rows */
  static const LogEst log15 = 39;  /* ~15 rows */
  if( nRow < log10 ){
    return SQLITE_TBL_SIZE_SMALL;
  }else if( nRow <= log15 ){
    return SQLITE_TBL_SIZE_MEDIUM;
  }else{
    return SQLITE_TBL_SIZE_LARGE;
  }
}
...
//todo: ads Added new code feature 2

         /* NEW: adaptive preference for INDEX-ONLY (covering) scans. */
        if( pNew->wsFlags & WHERE_IDX_ONLY ){
          LogEst nRow = pTab->nRowLogEst;             /* table size estimate */
          int szClass = sqlite3TableSizeClass(nRow);

          /* Small tables: nearly neutral. */
          if( szClass==SQLITE_TBL_SIZE_SMALL ){
            /* Optionally add a tiny penalty so choice is more neutral. */
            pNew->rRun += 1;
          }else if( szClass==SQLITE_TBL_SIZE_MEDIUM ){
            /* Medium tables: moderate preference for covering scans. */
            pNew->rRun -= 4;
          }else{ /* SQLITE_TBL_SIZE_LARGE */
            /* Large tables: strong preference for covering scans. */
            pNew->rRun -= 12;
          }
        }  
